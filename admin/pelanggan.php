...
// ==========================================
// GENERATE OTOMATIS NOMOR PELANGGAN (CS-XXX)
// ==========================================
$query_auto_bengkel = mysqli_query($koneksi, "SELECT nomor_pelanggan FROM tbl_pelanggan ORDER BY id_pelanggan DESC LIMIT 1");
if (mysqli_num_rows($query_auto_bengkel) > 0) {
    $data_auto = mysqli_fetch_assoc($query_auto_bengkel);
    $last_num  = $data_auto['nomor_pelanggan']; // Contoh: "CS-005"
    $clean_num = (int)substr($last_num, 3);    // Ambil angka "5"
    $next_num  = $clean_num + 1;
    $nomor_pelanggan_otomatis = "CS-" . sprintf("%03d", $next_num); // Jadi "CS-006"
} else {
    $nomor_pelanggan_otomatis = "CS-001";
}
...
// 1. PROSES TAMBAH DATA (Sesuai Kolom DB Anda)
if (isset($_POST['action_tambah'])) {
    $no_cust  = mysqli_real_escape_string($koneksi, $_POST['nomor_pelanggan']);
...
if (isset($_POST['action_edit'])) {
    $id_p     = mysqli_real_escape_string($koneksi, $_POST['id_pelanggan']);
    $nama     = mysqli_real_escape_string($koneksi, $_POST['nama_lengkap']);
    $nik      = mysqli_real_escape_string($koneksi, $_POST['nik']);
    $telp     = mysqli_real_escape_string($koneksi, $_POST['no_telepon']);
    $password = mysqli_real_escape_string($koneksi, $_POST['password']);
...
if (isset($_GET['hapus'])) {
    ...
    $query_del = mysqli_query($koneksi, "DELETE FROM tbl_pelanggan WHERE id_pelanggan='$id_hapus'");
    if ($query_del) {
...
<head>
    ...
    <style>
        ...
        * { font-family: 'Plus Jakarta Sans', sans-serif !important; }
        ...
        /* SIDEBAR PANEL FLOATING STYLE WITH SCROLL */
        .sidebar-panel { 
        ...
        /* MODAL STYLING PREVENT OVERRIDE */
    ...
    <div class="main-canvas">
        <div class="main-content">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h3 class="fw-bold text-dark m-0">Data Pelanggan Bengkel</h3>
                    <p class="text-muted small m-0 mt-1">Kelola seluruh master data pelanggan aktif di SIBEO.</p>
                </div>
            ...
            <div class="data-card-header">
                <h5 class="data-card-title"><i class="bi bi-people text-primary me-2"></i>Daftar Master Pelanggan</h5>
            </div>
                        ...
                        if ($query_pelanggan && mysqli_num_rows($query_pelanggan) > 0) {
                            while ($data = mysqli_fetch_assoc($query_pelanggan)) {
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge bg-light text-primary border border-primary-subtle px-2 py-1 fw-bold"><?= safe_text($data['nomor_pelanggan']); ?></span></td>
                                    <td><strong class="text-dark"><?= safe_text($data['nama_lengkap']); ?></strong></td>
                                    <td><span class="text-secondary"><?= safe_text($data['nik']); ?></span></td>
                                    <td><span class="text-secondary"><?= safe_text($data['no_telepon']); ?></span></td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <button type="button" 
                                                    ...
                                                    data-nopel="<?= safe_text($data['nomor_pelanggan']); ?>"
                                            ...
                                            <a href="pelanggan.php?hapus=<?= $data['id_pelanggan']; ?>" class="btn btn-sm btn-light text-danger fw-bold px-2" onclick="return confirm('Hapus data pelanggan ini?')">
                            ...
                            echo "<tr><td colspan='7' class='text-center text-muted py-5 small'><i class='bi bi-folder-x d-block fs-2 mb-2 opacity-50'></i>Belum ada data pelanggan di database.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold text-dark"><i class="bi bi-person-plus text-primary me-2"></i>Tambah Pelanggan Baru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="pelanggan.php" method="POST">
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-secondary">Nomor Pelanggan (Otomatis)</label>
                        <input type="text" name="nomor_pelanggan" class="form-control bg-light fw-bold text-primary" value="<?= $nomor_pelanggan_otomatis; ?>" readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nik" class="form-control" placeholder="Masukkan nomor identitas" required>
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold text-dark"><i class="bi bi-pencil-square text-warning me-2"></i>Ubah Data Pelanggan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="pelanggan.php" method="POST">
...
<script>
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('edit_nopel').value = this.getAttribute('data-nopel');
